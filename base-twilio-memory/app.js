...
const flowBoleitaNorte = addKeyword(['1'],{ sensitive: true }).addAnswer(
...
const flowLaCastellana = addKeyword(['2'],{ sensitive: true }).addAnswer(
...
const flowElBosque = addKeyword(['3'],{ sensitive: true }).addAnswer(
...
const flowLasMercedes= addKeyword(['4'],{ sensitive: true }).addAnswer(
...
const flowLosNaranjos = addKeyword(['5'],{ sensitive: true }).addAnswer(
...
const flowCatia = addKeyword(['6'],{ sensitive: true }).addAnswer(
...
const flowCaracas = addKeyword(['1'],{ sensitive: true }).addAnswer(
    ...
    {capture:true},
    null,
    [flowBoleitaNorte,flowLaCastellana,flowElBosque,flowLasMercedes,flowLosNaranjos,flowCatia]
)
...
const flowMiranda = addKeyword(['2'],{ sensitive: true }).addAnswer(
    ...
    (ctx,{fallBack,flowDynamic}) =>
    {
        if (ctx.body == '1') {
            ...
            if (myState.motivo === '1'){
                flowDynamic('Si deseas contactarnos para obtener una cotización, conocer nuestras promociones y verificar la disponibilidad de productos, este es el contacto de la tienda')
            } else if (myState.motivo === '3'){
                flowDynamic('Por favor comunicate con esta sucursal para recibir información referente a tu cotización deseada')
            } else if (myState.motivo === '4'){
                flowDynamic('Por favor comunicate con esta sucursal para recibir información referente a promociones')
            } else if (myState.motivo === '5'){
                flowDynamic('Por favor comunicate con esta sucursal para recibir información referente a disponibilidad')
            }
        } else if (ctx.body == '2'){
...
const flowTiendas = addKeyword(['1','3','4','5'], { sensitive: true }).addAnswer(
    [
...
const flowCatalogo = addKeyword(['2'], { sensitive: true }).addAnswer(
    [
...
const flowString = addKeyword('asd').addAnswer('Este mensaje envia tres botones', {
...
.addAnswer('¡Gracias por contactar a *Prosein*! 🙌🏼.')
    .addAnswer('Indíquenos tu Nombre y Apellido',{capture:true},(ctx,{fallBack,flowDynamic,state}) =>{
        state.update({nombre: ctx.body})
        const myState = state.getMyState()
    })
    .addAnswer('Indíquenos tu Correo',{capture:true},(ctx,{fallBack,flowDynamic,state}) =>{
        if (ctx.body.includes("@")) {
            state.update({correo: ctx.body})
            const myState = state.getMyState()
            flowDynamic(`Gracias ${myState.nombre} ${myState.correo} por tu informacion!`)
        }
    })
    .addAnswer(
