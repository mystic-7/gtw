  ...
  async (ctx, { flowDynamic, fallBack, state }) => {
    ...
    if (tienda === undefined) {
      ...
      return await flowDynamic(
        tienda.direccion +
          '' +
          motivacion.toLowerCase() +
          '.\n' +
          '\n' +
          (linkws === ''
            ? ''
            : linkws +
              `&text=Hola%20soy%20${nombreDeContacto.replace(/\s/g, '%20')},` +
              (correoDeContacto === undefined
                ? ''
                : '%20mi%20correo%20es%20' + correoDeContacto + '%20y') +
              `%20quisiera%20saber%20informaci%C3%B3n%20sobre%20${motivacion.replace(
                /\s/g,
                '%20'
              )}`)
      );
        ...
        return await flowDynamic(
          ciudad.direccion[0] +
            ...
            (linkws === ''
              ? ''
              : linkws +
                `&text=Hola%20soy%20${nombreDeContacto.replace(
                  /\s/g,
                  '%20'
                )},` +
                (correoDeContacto === undefined
